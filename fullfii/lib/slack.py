...
from fullfii.lib.constants import (
    SLACK_WEBHOOKS_URL_MSG_WARNING,
    SLACK_WEBHOOKS_URL_MSG_TABOO,
    SLACK_WEBHOOKS_URL_ROOM_WARNING,
    SLACK_WEBHOOKS_URL_ROOM_TABOO,
...
class SlackSender:
    ...
    def send(self):
        if self.settings is None:
            ...
            return False
        if not "slack_webhooks_url" in self.settings:
            print("slack webhooks URLが未設定です")
            return False

        requests.post(
            self.settings["slack_webhooks_url"],
...
class InappropriateMessageAlertSlackSender(SlackSender):
    """不適切アラート"""
    ...
    def generate_settings(self):
        # "タブー" or "警告"
        inappropriate_type__url = {
            InappropriateType.TABOO: SLACK_WEBHOOKS_URL_MSG_TABOO,
            InappropriateType.WARNING: SLACK_WEBHOOKS_URL_MSG_WARNING,
        }
        ...
        main_text = f"{inappropriate_type_icon[self.inappropriate_type]} *【{inappropriate_type_label[self.inappropriate_type]}】不適切メッセージアラート* {inappropriate_type_icon[self.inappropriate_type]}"
        pretext = {
        ...
        return {
            "slack_webhooks_url": inappropriate_type__url[self.inappropriate_type],
    ...
    def create(
        ...
        return InappropriateMessageAlertSlackSender(
            sender,
            room,
        )


class InappropriateRoomNameAlertSlackSender(SlackSender):
    """ルーム名不適切アラート"""
    ...
    def generate_settings(self):
        # "タブー" or "警告"
        inappropriate_type__url = {
            InappropriateType.TABOO: SLACK_WEBHOOKS_URL_ROOM_TABOO,
            InappropriateType.WARNING: SLACK_WEBHOOKS_URL_ROOM_WARNING,
        }
        inappropriate_type_label = {
            InappropriateType.TABOO: "タブー",
            InappropriateType.WARNING: "警告",
        ...
        main_text = f"{inappropriate_type_icon[self.inappropriate_type]} *【{inappropriate_type_label[self.inappropriate_type]}】不適切ルーム名アラート* {inappropriate_type_icon[self.inappropriate_type]}"
        pretext = {
            InappropriateType.TABOO: f" *{self.sender}* さんが以下のタブールームを作成/修正しようとしたため、自動凍結処理を行いました。メッセージが安全と判断された場合は、速やかに凍結解除を行ってください。",
            InappropriateType.WARNING: f" *{self.sender}* さんが以下の警告ルームを作成/修正しました。内容を確認し、凍結の検討・判断をしてください。",
        }
        fallback = {
            InappropriateType.TABOO: f"{self.sender}さんが以下のタブールームを作成/修正しようとしたため、自動凍結処理を行いました。",
            InappropriateType.WARNING: f"{self.sender}さんが以下の警告ルームを作成/修正しました。内容を確認し、凍結の検討・判断をしてください。",
        }
        attachment_color = {
            InappropriateType.TABOO: "#d9534f",
            ...
            "attachments": [
                {
                    ...
                    "image_url": "",
                    "fields": [
                        {"title": "【ルーム名】", "value": f"{self.message_text}"},
                        {
                            "title": f"【{inappropriate_type_label[self.inappropriate_type]}ワード】",
                            "value": f"{self.inappropriate_word_text}",
                        },
    ...
    def create(
        ...
        return InappropriateRoomNameAlertSlackSender(
            sender,
        )
